  ...
  return (
    <div className='flex gap-3 mt-5 mb-8'>
      ...
      <div className='grow mb-4'>
        <OutsideClickHandler
              onOutsideClick={() => setReplyOpen(false)}
        >
        ...
        <div className={`mt-4 ${replyOpen ? 'block' : 'hidden'}`}>
          <CommentForm buttonText={"Reply Comment"} commentId={comment.id} />
        </div>
        </OutsideClickHandler>
      </div>
    </div>
      ...
      <div>
        {comments.map((comment) => (
          <div key={comment.id}>
            ...
            <div className={`${test && 'ml-8'}`}>
              {comment.children && renderComments(comment.children, false)}
            </div>
          </div>
        ))}
      </div>
